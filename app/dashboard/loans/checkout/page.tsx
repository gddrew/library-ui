...
export default function CheckoutPage() {
  ...
  const [collapsed, setCollapsed] = useState<Record<number, boolean>>({}); // loanId -> collapsed?
  const [titles, setTitles] = useState<Record<number, string>>({}); // mediaId -> title
        ...
        {recent && recent.length > 0 && (
          <ul className='mt-3 space-y-3'>
            {recent.map((loan) => {
              ...
              const remaining = Math.max(0, loan.items.length - visible.length);
                      ...
                      <table className='w-full text-sm'>
                        <thead className='text-gray-500'>
                          <tr>
                            ...
                            <th className='text-left font-normal py-1'>Due</th>
                            <th className='text-left font-normal py-1'>
                              Returned
                            </th>
                            <th className='text-left font-normal py-1'>
                        ...
                        <tbody className='divide-y'>
                          {visible.map((it, idx) => (
                            <tr key={idx} className='odd:bg-gray-50'>
                              ...
                              <td className='py-1'>{fmtDate(it.returnDate)}</td>
                              <td className='py-1'>
                                <span
                                  className={`text-xs px-2 py-0.5 rounded border ${chipClass(
                                    it.status
